...
function post_views_for_wp_settings() { 
	...
	add_settings_field( 
		'post_v_forwp_field_pv_label', 
		__( 'Post Views Label', 'postviews-for-wp' ), 
		'PostViews_for_wp_pv_label', 
		'pv_for_wp_page', 
		'PostViews_for_wp_pg_p_section' 
	);
...
function PostViews_for_wp_posttypes_selector( ) {
 ...
 function PostViews_for_wp_pv_label(  ) {
	    $pv_forwp_opt = get_option( 'pv_forwp_settings' );
	    $pvlabel_val = $pv_forwp_opt['post_v_forwp_field_pv_label'];
	    ?>
	    <input class="input_pvfwp" type="textbox" name='pv_forwp_settings[post_v_forwp_field_pv_label]' value="<?php echo esc_attr($pvlabel_val); ?>">
	 	<p class="description_pvfwp">Enter the label for the post views counter field.</p>
	    <?php
	}
